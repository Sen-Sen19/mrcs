...
$(document).ready(function() {
  ...
  $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
            let firstMonthTableBody = $('#first_month_table_body');
            firstMonthTableBody.empty();

            data.forEach(function(row) {
                let newRow = `<tr>
                                <td>${row.process}</td>
                                <td>${row.machine_requirements}</td> <!-- Assuming you have this data -->
                                <td>${row.first_jph}</td>
                                <td>${row.wt}</td> <!-- Add the other columns as necessary -->
                                <td>${row.ot}</td>
                                <td>${row.mp_shift}</td>
                              </tr>`;
                firstMonthTableBody.append(newRow);
            });
        },
        error: function(jqXHR, textStatus, errorThrown) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function(data) {
            let tableBody = $('#first_month_table_body');
            tableBody.empty();

            data.forEach(function(row) {
                let newRow = `<tr>
                                <td>${row.process}</td>
                                <td>Machine Requirements Placeholder</td> 
                                <td>${row.first_jph}</td>
                                <td>WT Placeholder</td> 
                                <td>OT Placeholder</td> 
                                <td>MP Shift Placeholder</td>
                              </tr>`;
                tableBody.append(newRow);
            });
        },
