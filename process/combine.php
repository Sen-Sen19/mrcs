...
$sql = "
    SELECT 
        ...
        up.[cocripper],
        up.[quickstripping],
        -- Columns from non_machine_process
        nmp.[airbag_housing],
        nmp.[cap_insertion],
        nmp.[shieldwire_taping],
        nmp.[gomusen_insertion],
        nmp.[point_marking],
        nmp.[looping],
        nmp.[shikakari_handler],
        nmp.[black_taping],
        nmp.[components_insertion],
  ...
  op.[v_type_twisting],
    ...
    ON 
        fp.[base_product] = up.[base_product]
        AND fp.[car_model] = up.[car_model]
        AND fp.[product] = up.[product]
        AND fp.[car_code] = up.[car_code] 
        AND fp.[block] = up.[block]
        AND fp.[class] = up.[class]
        AND fp.[line_no] = up.[line_no]
        AND fp.[circuit_qty] = up.[circuit_qty]
    JOIN
        [live_mrcs_db].[dbo].[non_machine_process] AS nmp
    ON
        fp.[base_product] = nmp.[base_product]
        AND fp.[car_model] = nmp.[car_model]
        AND fp.[product] = nmp.[product]
        AND fp.[car_code] = nmp.[car_code] 
        AND fp.[block] = nmp.[block]
        AND fp.[class] = nmp.[class]
        AND fp.[line_no] = nmp.[line_no]
        AND fp.[circuit_qty] = nmp.[circuit_qty]

         JOIN
        [live_mrcs_db].[dbo].[secondary_process] AS sp
    ON
        fp.[base_product] = sp.[base_product]
        AND fp.[car_model] = sp.[car_model]
        AND fp.[product] = sp.[product]
        AND fp.[car_code] = sp.[car_code] 
        AND fp.[block] = sp.[block]
        AND fp.[class] = sp.[class]
        AND fp.[line_no] = sp.[line_no]
        AND fp.[circuit_qty] = sp.[circuit_qty]

        JOIN
        [live_mrcs_db].[dbo].[other_process] AS op
    ON
        fp.[base_product] = op.[base_product]
        AND fp.[car_model] = op.[car_model]
        AND fp.[product] = op.[product]
        AND fp.[car_code] = op.[car_code] 
        AND fp.[block] = op.[block]
        AND fp.[class] = op.[class]
        AND fp.[line_no] = op.[line_no]
        AND fp.[circuit_qty] = op.[circuit_qty]


    ORDER BY fp.[base_product]
    OFFSET $offset ROWS
...
if ($result === false) {
    die(json_encode(["error" => "Error fetching data", "details" => sqlsrv_errors()])); 
}
